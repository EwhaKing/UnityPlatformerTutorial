...
public class MovementEffects : MonoBehaviour
{
    ...
    [SerializeField]
    private ParticleSystem footStepEffect;//이동 효과 obj
    private ParticleSystem.EmissionModule footEmission;//footstepeffect의 emission module을 저장
    ...
    private void Update()
    {
        //플레이어가 바닥을 밟고 있고, 좌우 이동속도가 0이 아니면
        if(movement.IsGrounded && movement.Velocity.x != 0)
        {
            footEmission.rateOverTime = 30;//먼지입자 주기적으로 생성하기
        }
        else
        {
        ...
        //바로 직전 프레임에 공중에 있었고, 이번 프레임에 바닥을 밟고 있고, y 속력이 0이하일 때 바닥에 착지로 판단하고 이펙트 재생
